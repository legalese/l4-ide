...
const purifyJs  = readFileSync('node_modules/dompurify/dist/purify.min.js','utf8');
...
const res = await page.evaluate(async (src, ALLOWED_TAGS) => {
  ...
  let svg;
  try { ({ svg } = await mermaid.render('diagram', src, document.getElementById('mermaid-view-template'))); }
  ...
  el.removeAttribute('width'); el.style.width = '1500px'; el.style.height='auto';
  const styleKept = !!el.querySelector('style');
  const cssLen = styleKept ? el.querySelector('style').textContent.length : 0;
  return { rawLen, sanitizedLen: el.outerHTML.length, styleKept, cssLen,
           removed: DOMPurify.removed.map(r => r.element?.nodeName || r.attribute?.name) };
}, src, ALLOWED_TAGS);

if (res.error) { console.log('RENDER ERROR:', res.error); await browser.close(); process.exit(1); }
