...
> a new module with no edit to `StateGraph.hs` and no dependency on P2b/P2c/P2h.
>
...
> fixpoint over the reachable nodes (`solveDominators`, `Dominators.hs:165`). That is the
> formulation Cooper, Harvey & Kennedy, _A Simple, Fast Dominance Algorithm_ (2001) §2 start
> from; the answer is identical **by definition** — the dominator set of a node is unique, and
> every correct algorithm computes exactly it. The graphs are tens of nodes, so the near-linear
> engineering (theirs or Lengauer–Tarjan's) buys nothing here. Two adaptations: (i) acts are
> **edges**, so every transition is subdivided into a node of its own and the answer is read
> off the edge nodes; (ii) **`RAND` has no join in the IR** (R2), so a literal walk says
> neither branch of a conjunction dominates `Fulfilled`. `fulfilmentView` (`:196`) rewrites
> each `AllOf` so its branches run in sequence — branch _k_'s arrivals at `Fulfilled` are
> re-pointed at branch _k+1_'s entry — which has the same set of edges on every path to
> `Fulfilled` as the concurrent contract has on every run, since dominance is order-blind.
> The view is used only for a `TerminalFulfilled` target; breach and intermediate states are
> answered over the literal graph, which is already right for them (one branch failing
> breaches the whole; an intermediate state lies inside one branch).
>
...
> DOT output is byte-identical (asserted by `l4-cli-test`, "without --dominators the DOT
> output is unchanged"). The DOT annotation ("bold the dominating edges") is **not built**:
> it needs an option on `StateGraphOptions`, which is an edit to `StateGraph.hs` this branch
> was told to avoid while a fix to that file is in flight.
>
...
> `jl4-core/test/DominatorsSpec.hs` (19 examples, 0 failures): a chain dominates its sink with
> every act; `ROR` with nothing; `RAND` with **both** branches; `(a ROR b) RAND c` with `c`
> alone; `(a RAND b) ROR c` with nothing; a renewing duty (`HENCE` to self) terminates and
> only its timeout reaches breach; a hand-built unreachable state answers `Unreachable`.
>
...
> route is live. The user page (`doc/reference/regulative/state-graph.md`) says so.
...
simple, fast dominance algorithm", Rice CS TR-06-33710, 2001 (the iterative formulation P2f
implements, §1.1c). Sugiyama, Tagawa & Toda, "Methods
