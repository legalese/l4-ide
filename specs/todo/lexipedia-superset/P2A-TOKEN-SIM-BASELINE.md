...
- **Run:** `cd etc/bpmn-token-sim && npm install && npm run build && npm run run`, 2026-09-15
  08:45 UTC. Four full runs, identical results.
...
means it comes from the simulator's source at `node_modules/bpmn-js-token-simulation@0.40.0/lib/…`
...
| deadlines as boundary timers                       | Five timers, all clickable at once, none counting. `P21D`, `P7D`, `P30D`, `P120D`, `P365D` are labels; a reader cannot tell from the animation that the resale restriction outlasts the filing by a year.                                                                                                                                                                                                                                                        | MEASURED |

Screenshots: `offering.png` (token on `Task_0`, five timers), `offering.breach.png` (after `P21D`:
...
### 3.6 `regcf-reporting` — the renewing obligation, `P-CYCLE`, a mixed gateway, three arms
...
| the `Mixed` gateway               | bpmn-js renders it and the simulator routes through it (jBPM rejects it — `README.md`). Its three arms are one setting cycled by clicking; arm 1, "fulfilled", is `Split_0 → End_2` with no task in between, so on that arm the process finishes after one click on `Decide_0` and _nothing was ever owed_ — which is right, and indistinguishable from a process that was skipped.                                                                         | MEASURED |
...
| time                                           | —             | None. No clock, no countdown, no ordering of deadlines; a `P365D` and a `P7D` timer are equally clickable at t = 0.                                                                                                                                                                                                                                                                      | MEASURED |
...
- **Concurrency.** Four tokens in four lanes on `offering` is a genuinely better answer to "what is
...
- `etc/bpmn-token-sim/` — `package.json`, `src/app.js`, `index.html`, `build.mjs`, `run.mjs`,
  `README.md`, `.gitignore` (`node_modules/`, `dist/`, `package-lock.json` ignored; `out/`
  un-ignored).
