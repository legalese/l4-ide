...
| `D-SVCEMPTY`         | added 2026-08-02 (R12's companion, DMN spec §7) | Advisory         | `Faithful`    | no `<decisionService>` for a §, said out loud. `Advisory ⟹ Faithful` (§3.2) is discharged: arm (i)'s per-decision loss is already carried at Lossy by each member's `D-RULEDATE-UNBOUND`, arm (ii)'s member decisions are ALL emitted — only the grouping shell is not, and an uninvoked grouping service is inert on both engines (probe D1, `D-FLAVOR-NOSERVICE`'s grouping-arm precedent). One severity, two message forms (`D-FIXTURE`'s precedent).                                         |
