...
- **M1.5 — Bare-cell READS via the prelude `Dictionary`.** The snapshot is materialised at the L4 boundary as the prelude's **`Dictionary k v`** (`prelude.l4:737`), so a read desugars to `dictLookup <cell> <env-snapshot>` → `MAYBE v`. This reuses a built, tested type and its kit (`dictLookup`/`dictFindWithDefault`/`dictKeys`) instead of bespoke machinery — the same dividend M3 gets from the NAF combinators. Open UX fork (see §9): a *bare* cell `` `X` `` requires renamer/schema work to distinguish a cell from a variable; an *explicit* `dictLookup "X" env` over a materialised snapshot needs none. Keep the Haskell `Map` for the internal snapshot; only materialise a `Dictionary` **value** at the read boundary (assoc-list is O(n), fine for dozens of cells).
...
- **M3 — Presumption / `resolve` (D3).** Read-before-write returns the cell's declared presumption — which is **exactly `dictFindWithDefault <presumption> <cell> <env>`** (`prelude.l4:838`, `= fromMaybe default ∘ dictLookup`). The just-landed NAF combinators (`holds = fromMaybe FALSE`, `presumed = fromMaybe TRUE`, `a930f62c`) are the boolean specialisations of that same `fromMaybe`. So M3 is *no new machinery*: the env cell's `resolve` is `dictFindWithDefault`, and the cell's declared default (the monoidal identity, §5) selects the presumption. Unify with `WithDefault`/`TYPICALLY`.
- **M4 — Per-party ledgers + `COMMIT`/`ATTEST` (R1) and `local` quarantine (Q5).** The concurrency story; needs design ratification (§8, §11) — do last.
