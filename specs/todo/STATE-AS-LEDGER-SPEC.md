...
- **M1 — Surface syntax.** Lexer tokens `RECORD` / `COMMIT` / `ATTEST` and the `IS` copula (no `:=`); parser production for `RECORD <cell> IS <expr>` (and `COMMIT`/`ATTEST`) as a HENCE/LEST-followup statement; AST node `Record` (sibling of `Fetch`/`Post`/`Env` in `Syntax.hs:200`), with a ledger-target flag (own vs official); a bare cell `X` reads via snapshot, a cross-party `Party's X` reuses the genitive `'s` accessor; rename + typecheck the cell path against an `EnvironmentState` schema (declared for M1, inferred in a later pass — R2). Desugar to the M0 substrate.
...
- **M3 — Presumption / `resolve` (D3).** Read-before-write returns the cell's declared presumption. **Reuse the prelude NAF combinators just landed in `a930f62c`** — `holds = fromMaybe FALSE`, `naf`, `presumed = fromMaybe TRUE` in `jl4-core/libraries/prelude.l4` — which already *are* the closed-/open-world `resolve` over a `MAYBE BOOLEAN`. No new machinery; the env cell's `resolve` is exactly `holds`/`presumed`, and the cell's declared default selects which. Unify with `WithDefault`/`TYPICALLY`.
...
2. **Cell identity / paths.** Is a cell path like `` `freezing point of water` `` a flat string-keyed map, or a typed path into a nested `EnvironmentState`? (Bears on §9 and on whether two parties' "same" cell are identical.)
