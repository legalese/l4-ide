...
#### P10.1 — The destination is computed. RULED 2026-09-15; the branch RE-RULED 2026-09-26.
...
| branch     | `main`                                             | **ruled 2026-09-26** — see below; `L4_GO_CANON_BRANCH` overrides |
| directory  | `subjects/<subject_path>/encodings/<encoding-id>/` | the sidecar's `canon.subject_path`, and the run's encoding       |

**The branch is `main`. RULED 2026-09-26 (Meng).** "From now on, let's commit straight to main."
Committing to canon's `main` needs membership of the `legalese` GitHub organisation, and "right now
the only audience … is internal to legalese, so that's fine"; "non-legalese contributions are
welcome and should follow standard GH fork/pull-request protocol", so a contributor outside
Legalese sets `L4_GO_CANON_BRANCH` (or `--branch`) to a branch of their fork. Meng: "if people start
noticing canon, we can revisit this to be more scalable to external contributions."
...
carries the destination **path only**; the branch is resolved at deposit time.
