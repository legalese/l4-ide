...
deposited and report `SKIPPED`/`DEGRADED`/`PASS` with an exit code (ORCHESTRATOR.md §5.2). P8 and
P10 remain scaffolded entry points that refuse with a named blocker. `g2 COMPLETE` is completeness
...
| orchestrator ("go")        | **milestones G1 and G2's deposit half run today** — `etc/go/go.sh run --milestone g1 --subject regcf` drives the committed corpus through every reachable projection and emits report v0; `--milestone g2` runs P1, P2, P3-encode, P4 and P5 as stages that VALIDATE an agent-produced deposit (2026-08-03), leaving P8 and P10 scaffolded and refusing with a named blocker. Producing the deposits, and the §8 acceptance diff, stay agent work. Present-tense inventory: [ORCHESTRATOR.md](./ORCHESTRATOR.md) |
