...
| -------------- | ------------------ | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `p0-preflight` | PASS               | structural   | CLI enumerations and the module's regulative-rule names match the subject's `pins.json` as sets (the export formats as a subset: each pinned format must still be offered); every checker in the pin exists; the failing-`#ASSERT` tripwire still exits 0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `p7-catala`    | `l4 catala` per module, then `etc/validate-catala.mjs`: `catala typecheck`, `catala proof` (NoOverlappingExceptions), and `clerk test` re-executing every emitted ```catala-test-cli block | added 2026-09-21. `PASS`/`execution` when all three layers pass with a non-zero test count; `DEGRADED` when the emitter refuses, when Catala rejects an emission `l4 catala` exited 0 on, or when an emission carries no executable test; `SKIPPED` when the Catala toolchain is absent. MEASURED 2026-09-21 on subject `chubb`: `DEGRADED` — `l4 catala` refuses, naming five helpers with no `GIVETH` |
