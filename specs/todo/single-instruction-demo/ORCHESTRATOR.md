...
| `p9-cost`      | the ledger's own parts re-summed against the totals it states, and each interval union checked against the bounds a union must satisfy              | `structural`. Two standings in one artifact and they are never merged: the per-stage timings are the DRIVER's, and `ledger.verify` refuses one larger than the bracket it sits in; the token and tool figures are read from the agent harness's transcripts, each named with its sha256 |
