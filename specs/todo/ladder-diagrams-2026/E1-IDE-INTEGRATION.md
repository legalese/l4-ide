...
| **4** | **The `LadderSvg` displayer — now "a `ladder-svg` controller plus a thin `.svelte` shell". ⚠️ AMENDED 2026-07-27, see the note below the table.** The DOM half (the `{@html}` sink, the **one** delegated click on `data-value`/`data-fold`, the hand-rolled `viewBox` pan/zoom, the FLIP) goes in `ladder-svg/src/controller.ts`, **not** in the `.svelte` file, and the FLIP is _moved_ out of `standalone/app.ts:92-100` rather than copied. Still Step 4's job, unchanged: header shows the **verdict**, not `evaluates to …`; **reuse `partial-eval-sidebar.svelte` as-is** — widen its prop to a structural interface (~5 lines) and the whole "Still Needed" panel comes along. | L       |
