...
- **Rank imports by their own depth.** Arbitrary: it would make a top-level export of a
  ...
  silently repick overloads in the 282 files that import the prelude.
...
No evaluated answer moves anywhere. The golden suite is itself the evaluation differential
here: a `.golden` captures the `Result:` of every directive in its file (see
`ok/tests/logic.golden`), and the goldens in the tree were generated by the pre-change compiler.
Run against them, the changed compiler gives **2815 examples, 2 failures** — the two lines
above and nothing else, across roughly 300 goldened files including all 282 that import the
prelude.
