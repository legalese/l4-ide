...
- **Double-report is accepted, not suppressed.** On a clause that is _also_ an
...
- **The check assumes the convention `party-type == actor-index-type`.** It
  compares the party's _type_ to the action's index. So a model that types
  parties by a **separate union enum** (e.g. `DECLARE Party IS ONE OF …` with
  actions phantom-indexed by _different_ role types) will see agreement fire on
...
done). The **next rung** is _party / action-index agreement_: "an actor may only
...
contract with _union_ party/action types must still pass the ball Renter →
...
"threatening to go Liquid" for years); _"just GADTs might be easier if they
achieve our goal."_
...
relation between **two type arguments** of `DEONTIC a b` — _not_ between a runtime
...
- The actor index is **already recoverable by ordinary rank-1 HM**. `inferConDecl`
  ...
  dependent types, no GADTs, no constraint store needed to _observe_ the index.
...
metavariables independent. The Idris/Agda result that _generative datatypes ⇒
reflexive nominal index equality is decidable, total, and SMT-free_ is the formal
...
> GADTs are the right _ceiling_ if we later want more (see §5), not the next step.
...
- `TypeCheck.hs` (`prettyTypeMismatch`, ~`3486`): add an arm rendering a
  **domain phrase**, e.g. _"In this obligation the party is an Eater, but the
  action `Drink` belongs to a Drinker. An actor may only be obligated to perform
  its own actions."_ This is the single biggest UX lever — pick the `expect`
...
- Tests: the pending fixture flips to satisfied (regenerate its
  ...
  today) so we pin _accept-on-match_, not only reject-on-mismatch.
...
### 2.3 Honest boundaries (what this step does _not_ do)
...
1. **Only fires when the action carries an index.** Plain-enum actions (the whole
   ...
   different encoding (party and action drawn from a shared actor _kind_) — out of
   scope.
...
3. **Actor-indexed multiparty residuation is already in tension** — independently
   ...
   is to index by the _union_ actor (`DEONTIC Actor (Action Actor)`), where
   agreement is a happy no-op.
4. **No value-level identity.** It cannot say "_this_ `AsRenter` token may only do
   _this_ renter's action" — that is a value-dependency (§5 ceiling).
...
- **"actor = first index" is an unstated, fragile convention.** A coherent
  `DEONTIC Eater (Action Food Eater)` (actor as the _second_ index — natural for
  ...
  action heads, _or_ add an explicit actor-index marker on the action declaration,
...
- **Diagnostic collision / double-report.** On the rung-1 fixture
  ...
  original action-type mismatch _plus_ the new agreement error). Both are true,
...
- **LiquidHaskell** is the canonical bolt-on: refinements in comments, a _separate_
  ...
  messages** (raw solver noise). _Transfer:_ wrong altitude for L4's first move;
  right _architectural_ lesson — keep any heavier checking a **separate, opt-in
...
- **Dependent Haskell / Singletons / DataKinds** — the "slow burn." Using a
  ...
  "inefficiency," "accidental complexity." _Transfer:_ strong evidence **against**
...
- **GADTs in GHC** buy exactly per-constructor index refinement + per-branch
  ...
  sites. _Transfer:_ L4 already **sidesteps the worst of this** — `inferContract`
  (`TypeCheck.hs:1488`) gives every clause a fully-known declared type via
  `GIVETH A DEONTIC …`, so a GADT-style indexed-action scheme would _always_ be
  checked against a user-written signature, dodging the principal-types problem.
  So GADTs are the right **ceiling** if the nominal rule proves too weak — _not_
  singletons, _not_ SMT.
- **Idris/Agda** indexed families decide index equality by **reflexivity** on
  ...
  and errors local. _Transfer:_ this is precisely the cheap path §2 takes, and it
...
- **F\*/Dafny** show the heavy end's usability cost: SMT **brittleness**
  ...
  CNL whose selling point is **traceable, citable, local** reasoning. _Transfer:_
...
1. **Rung 1 — action-type enforced** _(done)._ `MUST`/`MAY` action checked deeply
   against the contract's declared action type. Elaboration fix, no new theory.
2. **Rung 2 — party/action-index agreement** _(this spec, recommended now)._
   One nominal-equality constraint. Stays at HM. Cost S.
3. **Rung 3 — GADT-style indexed actions** _(only if needed)._ Per-constructor
   result-type refinement (two constructors of one `DECLARE Action` fixing
   _different_ closed indices) + refinement-by-matching in `CONSIDER`. Needs:
   ...
   matching must _learn_ the index.
4. **Ceiling — value-level dependency** _(probably never in the core)._ "This
...
# Addendum — Rung 3: actor-correct _free_ residuation ("the ball in either court")
...
> acceptance test on the live binary. The sketch's _diagnosis_ (the residual
> pin, the union/specific fork, the GIVETH-rigidity dodge) held; its _cost
> framing_ ("XL — first-class `Exists`") and its _load-bearing piece_
...
residuation present only as a _preserve_ constraint. The **union encoding
satisfies that constraint vacuously** (`DEONTIC Actor (Action Actor)` residuates
_and_ passes agreement `Actor ~ Actor`), so nothing forced the two goals to be
...
- **Union index** `DEONTIC Actor (Action Actor)` — residuates Eater→Drinker
  ...
  and `PARTY ADrinker MUST eat` is _accepted_.
- **Specific index** `DEONTIC Eater (Action Eater)` — keeps agreement's teeth,
  ...
  _"HENCE … expected DEONTIC OF Eater, Action OF Eater but is here DEONTIC OF
  Drinker, Action OF Drinker."_

So under _nominal_ typing, **"routes the ball between actors" and "each actor only
...
Index _both_ party and action by the actor, and have each obligation
...
- The **contract type mentions no actor**, so residuals may use a _different_
...
3. **Residual unification (`ExpectRegulativeFollowupContext`):** HENCE/LEST must
   unify against the existential contract type, instantiating `who` _per residual_
...
5. **Evaluator (EvaluateLazy.hs):** largely unaffected — `ValObligation` stores
   party/action as runtime _values_, never types, so the existential is erased at
...
- **Singletons / value→type** (the party _value_ determines the action type) —
...
Actor)` + a guard that the party and action denote the same actor) — **zero
  type-system change, available today** — gives free residuation + _dynamic_
...
- **Rung 3 (when a concrete case needs _static_ actor-correct routing):**
...
was confirmed where it described _today's_ behaviour and corrected where it
guessed at _cost_ and _mechanism_. Net: **the static refinement is cheaper than
the sketch claimed (S–M, not XL), but "the ball in either court" at _runtime_
...
- **The pin is `checkDeonton`'s `let rTy = contract partyT actionT`
  ...
  the head actor. Confirmed even with _no_ `GIVETH` signature (the first
...
- **The union/specific fork is real and mutually exclusive under nominal typing**
  ...
  vacuously accepts `PARTY ADrinker MUST eat` (_"Check succeeded"_); specific
...
- **Full dependent types / SMT (the "going Liquid" route) is overkill** — the
  per-clause obligation is one _nominal equality_ (party type ~ action's actor
  index), not a refinement predicate. The LiquidHaskell decade-long slow burn is
  the cost of the _SMT/refinement substrate_, which this problem never invokes.
...
showed both halves of the acceptance test pass _as a static type-check_. Then it
**drove the contract with events** and found the fatal flaw:

> The `#TRACE`/`#CONTRACT` directive types the event stream as
> `eventT = event partyT actionT` (**TypeCheck.hs:437**) with a _single_ actor
...
> _"EVENT OF Eater, Action OF Eater but is here EVENT OF Drinker, Action OF
> Drinker."_
...
executable rung-3 routing you must relax _both_ seams coherently** (residual _and_
...
clause** — party and action are leaves, and obligations are _never destructured
downstream_ the way a GADT scrutinee is. **No elimination site ⇒ no escape hazard
to police.** The "existential" is therefore _operational_, not a type: a fresh
...
sketch applies only to the _first-class-`Exists`_ route (general `∃` constructor +
...
(_"expected 2, found 0"_, plus a confusing double-report); (b) more fundamentally
...
The obvious one-line version — fresh _both_ party and action per residual — is
...
### 5. The reject-half needs per-actor party _types_ (rung-2's known limitation).
...
- **Interim, available today:** runtime `PROVIDED` guard on the union encoding
  ...
  today** — but actor-correctness is _dynamic_, and the union footgun (vacuous
...
_distinct_ `w`; `PARTY ADrinker MUST eat` is rejected because _within one clause_
`w ~ Drinker` (from the party) and `w ~ Eater` (from `eat : Action Eater`) both
fire on the same `w`, and `ensureSameRef Drinker Eater = False`.

> **Caveat that sank a rival proposal — it must be a `fresh` _metavar_, not a
...
> yields a `TyApp who []` that `ensureSameRef` unifies only with _itself_. Under
> that, `expect (party = Eater) who` would **fail even in the good case**
> (`Eater MUST eat`). The actor index here must be _solvable_ (a metavar the
...
MUST Drink`) that now type-checks — and a NOT-OK fixture — that same style with a
   residual `PARTY ADrinker MUST Eat` rejected statically. _That is the acceptance
   test, encoded as fixtures._
...
4. **Separately scoped (M, do _not_ block step 1):** decide whether routed
   contracts must be _driveable by cross-actor events_. If yes, relax the event
...
who`/0-arg head (arity error today + starves the event type); freshening _both_
...
_"rigid type variable would escape its scope"_ — pure PL jargon at the
