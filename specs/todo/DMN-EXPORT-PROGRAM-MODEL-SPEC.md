...
| `D-REFUSE`           | **RULED 2026-09-05 (`IMPLICIT-PROPS-DESIGN.md` §11.9.1, amending R7), NOT BUILT.** One per decision whose lowered body contains a reachable `REFUSE`. The refusal lowers to FEEL **`null`** and **no row is omitted**; the reason string goes to the surviving `OTHERWISE` row's `<description>` and is named in the note. A DRG carrying any `D-REFUSE` **withdraws `DMN-SAFE`**, and there is deliberately **no `MayRefuse` safety kind** — a kind that does not withdraw `DMN-SAFE` certifies a decision total when we can see it is not. **Two severities, taken knowingly**, set by the existing strictness calibration: `Lossy` when the refusal is consumed only from lazy positions, `Blocking` from any strict consumer. Per this section's header that is the effect axis smuggled into severity, as in `D-PARTIAL` and `D-FLAVOR-NOSERVICE`, and it **inherits `D-PARTIAL`'s migration obligation verbatim**: when FIDELITY W1/W2 lands the two collapse to one severity plus one `FidelityEffect`. The **dmnmd/Markdown carrier is a second image and is NOT ruled here**: `mdOutput` accepts S-FEEL only, so a `null` cell reaches `D-MD-CELLSYNTAX` (`Dmn/Markdown.hs:404-412`) rather than rendering. **What ships today is neither this image nor the superseded one**: `Refuse {}` lowers to `verbatim e` (`jl4-core/src/L4/Dmn/Lower.hs:2285`), writing L4 source text into a FEEL literal, and KIE 8.44.0.Final then fails to compile the whole DMN file (`ERR_COMPILING_FEEL`, measured 2026-09-05). Three conditions land with the code and are stated in §11.9.1, not here.                                                                                                                                                                                                                                                                                                                                                                     | Omission and an explicit `null` are engine-identical under **both** hit policies — the `UNIQUE` `defaultOutputEntry` at `Lower.hs:695` and the dated-chain floor row at `Lower.hs:1645-1655` — so `PROPS-REDTEAM-2026-09-03.md` §2.8's "`REFUSE` → `null` would launder" cannot choose between them. `null` is then the cheaper half, and the only one that keeps the reason string in the artifact: omitting the row deletes the `<rule>` whose `<description>` would carry it.                                                                                                                                                                                                               |
