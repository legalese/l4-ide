...
| L11 | a **projection `x's f` whose scrutinee type is an `IS ONE OF` with more than one constructor**, unless every constructor of that type declares a field `f`                                                                                                                                                                   | `PartialSelector` — the source L1 cannot see                                                             |
...
ruling, and the implementation owes a test that fails when a new constructor appears
(`jl4/tests/DmnExport.hs`, §7.7's tripwire).

| constructor                 | foreclosed by                                                                        |
| --------------------------- | ------------------------------------------------------------------------------------ |
| `NonExhaustivePatterns`     | L1, L2 — see the note below                                                          |
| `PartialSelector`           | **L11**                                                                              |
...
**The note the map's first row points at, and the one time the tripwire has fired (2026-09-09).**
This map used to read _"`NonExhaustivePatterns` — L1, L2, **L11** — L1+L2 alone do not cover it"_,
because L11's site — a partial selector — is a `CONSIDER` the EVALUATOR synthesises, and falling off
it was reported under the same constructor as a drafter's own incomplete match. It now raises its
own, `PartialSelector`, so that the run-time message can name the field and the constructors that
declare it instead of describing a `CONSIDER` nobody wrote (SUM-TYPE-FIELDS-SPEC §1.1). §7.7's
9th-constructor tripwire fired on that split, exactly as designed, and the answer was that **L11
already had the clause**: nothing in DMN-SAFE's coverage changed, and the map got sharper, not wider
— L1 and L2 now cover `NonExhaustivePatterns` on their own, because the one route to it they did not
cover is no longer spelled that way.
