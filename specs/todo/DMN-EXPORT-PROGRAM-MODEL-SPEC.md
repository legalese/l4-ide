...
outputs/encapsulated guards — the D5 skip that §2.3 documents, still silent as of this ruling;
`D-SVCEMPTY` is the planned companion in the same PR that makes it loud, not landed with R12
itself) and `D-FLAVOR-NOSERVICE` goes 7 → 6; `D-MD-NODRG`'s requirement count moves
