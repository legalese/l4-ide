...
A fourth case exists and is **not** a "don't know" — it is a **wrong answer**, and unlike cases 1–3
it does not read as `¬TOTAL`. **Multi-clause pattern-matching definitions are exempt from L1
entirely**, so a genuinely incomplete clause group is certified DMN-SAFE and exported as a decision
table with a missing rule, silently. This is a consequence of suppressing the exhaustiveness oracle
inside the desugarer's `__pm_fallthrough_<k>` bindings; it was measured on 2026-07-31 and is
**OPEN — see §14.7**. Until it is fixed, DMN-SAFE means what this section says only for definitions
written as a single clause with an explicit `CONSIDER`.
...
> **record-construction subset of `AppNamed` now lowers to a FEEL context literal**
> `{f: e, …}` in declaration order — gated to RECORD constructors (an enum's payload constructor
...
> rows moved:
>
> - **p3 (self-recursion)**: the self-edge is no longer erased — one `D-CYCLE` ("requires
>   itself"), and the emitted file carries the `requiredDecision` self-edge it reports.
...
> - **p2/p4/p6/p7/p8/p9**: every decide-level requirement cycle now carries one `Blocking`
>   `D-CYCLE` naming every member by name and id. `cycle-p4-where-local-self` stays finding-free
...
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- |
| **0**   | Kill the dot passthrough in `feelIdentText` (§5.1-1). Fix `drgNamed`'s type-error hole (§9). Add the Xerces check. **+R5 (§6.4): LANDED 2026-08-01, as the first commit of the Phase 5 PR rather than as its own Phase 0 PR** (build-plan P5.0b records why it could not lag Phase 5: `knowledgeRequirement` edges are built from the same `freeRefs`): `checkDrg` + `D-CYCLE`; `freeRefs`' own-name binding and `classifyRef`'s self-edge filter deleted. The "expected golden delta is zero" prediction was measured **false by one**: §6.4.8's reasoning covered the one `.dmn` golden of its day, and the corpus golden added since carries a genuine self-recursion (`ongoing reporting obligation`), so the delta is one self-edge + one `D-CYCLE` note (§6.4.2, 2026-08-01 note).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | —          |
| **0.5** | **LANDED 2026-07-31.** PR #45's exhaustiveness oracle (`constructorsInScopeFromEntityInfo`, `@nonexhaustive`) reached this line twice and was reconciled by the merge of `origin/unstable` on 2026-07-31: vendored onto the Phase 4 branch as commit `904192ea`, and independently cherry-picked onto `unstable` from `8a8b46bc`. `8a8b46bc` is an ancestor of neither `unstable` nor `main` — `main` reverted the whole PR in `9ec57df0` and re-landed only a narrow `builtinConstructorLookup` in `7531f1d9`, so a `main` merge would NOT have delivered the oracle, and ancestry is not the right probe for it. Hard prerequisite for §2.4's L1 and L2; without it the totality criterion is vacuous outside same-file top-level enums. **Carried defect, measured 2026-07-31 — see §14.7.** Multi-clause pattern matching (`__pm_fallthrough_<k>`, `Parser.hs:952`) compiles its last clause without an `OTHERWISE`, so the bare oracle warns on definitions that are total at source level. The Phase 4 line suppresses the oracle inside fall-through Decides, which buys clean goldens on `ok/pattern-matching.l4` and `ok/pattern-matching-nullary.l4` at the price of a **false negative, not a conservative over-approximation**: a group of n ≥ 2 clauses is now exempt from the oracle entirely, incomplete ones included. Neither side of this trade is correct; the fix is to analyse the clause group as a unit before desugaring. **Open — see OPEN-6.** | —          |
...
**Status: OPEN. Measured 2026-07-31 during the `origin/unstable` merge; not fixed here, because
fixing it is a change to the desugaring or to the oracle, not a merge resolution.**
...
recovered. Until then §2.4's `DMN-SAFE` is **unsound for multi-clause definitions**, and that is
the one certification claim in this document that a reader must not rely on. A regression test is
deliberately **not** added here: pinning the present behaviour in a golden would make the fix
harder to land.
