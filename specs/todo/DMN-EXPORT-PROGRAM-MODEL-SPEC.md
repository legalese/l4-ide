...
| `D-SVCEMPTY`         | **New 2026-08-02 (R12's companion)**, `Advisory`, one severity, TWO message forms (`D-FIXTURE`'s precedent). A `§` for which no `<decisionService>` is emitted, named by the §'s FEEL-folded name (never by a service id the artifact does not contain). Arm (i): every member decide was rule-date-rebind-dropped (§15.12). Arm (ii): the struct cannot satisfy DMN 1.3 §6.3.10's shape (≥1 output decision, and ≥1 encapsulated member or input decision) — the probe-D5 skip that used to be silent; the note is new, the behaviour is not. **[E]** corpus: arm (ii) fires 8× (Thresholds, Periods, §2, §6, §7 — whose self-edge-carrying `ongoing reporting obligation` leaves it with NO output decision — Fixtures, Group 4, and the rule-version §); arm (i) is fixture-only (`not-ok/svc-empty-ruledate.l4`). Advisory ⟹ Faithful is argued in `FIDELITY-SEVERITY-AXIS-SPEC.md` §5.2: the per-decision loss is already carried by `D-RULEDATE-UNBOUND` (Lossy), and the shell is grouping metadata (`D-FLAVOR-NOSERVICE`'s grouping arm).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | a silent whole-element skip is how a reader is told a § never existed; every other drop in this exporter is named, and this one now is too.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
**Measured on the corpus at build time (goldens, 2026-08-02):** the 15 rebind `<decision>`s leave
`regcf-corpus.dmn` (82 → 67 decisions); the `inputData` population is unchanged at 15; the
rule-version `§`'s decisionService leaves the artifact with them (its only outputs were the 15; the
five kept helpers have no internal requirement edges, so the struct fails §6.3.10's
outputs/encapsulated guards — the D5 skip that §2.3 documents; `D-SVCEMPTY` — landed 2026-08-02
with R12's companion change — now names every such skip, and on the corpus its arm (ii) fires 8×,
see the §7 register row) and `D-FLAVOR-NOSERVICE` goes 7 → 6; `D-MD-NODRG`'s requirement count moves
224 → 153 and 15 `D-MD-NOLITERAL` Blocking rows leave the markdown report. `D-BKM` notes still name
rebind-dropped callers in their applied-by lists — **accepted deliberately**: the note records the
tier evidence, and tier classification runs on the pre-drop call graph by design (§2.1).
