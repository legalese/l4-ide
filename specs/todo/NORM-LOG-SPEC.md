...
> The rulings are posed with evidence, recommendations and an independent skeptic's objection on a bench published 2026-09-26 (claude.ai artifact `VW7xe4F73mi5qLST8PB1CJ`, private, marks in its db collection `norm-log-rulings`); a ruling is recorded here, not there.
...
3. _The clock._ A `HENCE` continuation's clock is the stamp of the event that reached it; a `LEST` continuation's is the deadline that was missed (`clockAt isHence stampR = if isHence then stampR else deadlineR`, `Machine.hs:2194`).
...
- **Reads are as-of the reader's own contract clock**, so an entry stamped later than the reading norm's clock is invisible to it. This does **not** remove the ordering problem between the operands of a `RAND`/`ROR`, because each operand runs over the whole trace before its sibling starts (N10).
...
| id      | question                                                                   | notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **N1**  | **Which transitions become entries?**                                      | Log a norm's **fates**, not its looks at events. `PartyMismatch`, `ActionMismatch`, `GuardFailed` and `EarlyAct` are looks, O(events × norms); `Waiting` is written once per residual norm when the trace runs out (`Machine.hs:1961-1964`). The vocabulary must cover all four modals, including `DO` (`DDo`, `Syntax.hs:668`), whose missed deadline is a failure as `MUST`'s is. Candidate fates: armed, performed, failed (to reparation / to breach), violated (a prohibition broken), kept (a prohibition's window closed), lapsed (a permission's window closed), breached, and the join outcomes. `Armed` is not a `StepOutcome` today.                                                                                                                                        |
...
| **N4**  | **Implicit write, explicit read — and in what surface?**                   | Proposed: the write implicit, the read explicit, as an extension of the anchor family (§5). An implicit read would make every expression potentially history-dependent, so a reader could not see which provisions depend on history. (Caching is not the obstacle: context reads are already fingerprinted, `Machine.hs:1039-1068`. But ledger reads are re-served within a temporal scope — "the fingerprint does not track the ledger", `:1053-1063` — so a fate read's cache key must include the reader's contract clock.) Also decides whether a time window is written in the read (shape (b)) or built in, and must give a guard a name for its own contract-clock instant, which it lacks today: `NOW` exists and type-checks in a `PROVIDED`, but it is the wall clock (§7). |
...
| **N9**  | **Waiver, release and pardon.**                                            | "Continuing" means not remedied **or waived** (§2 item 1); pardon is the waiver of shape (c). Waiver is an act of the other party, not a transition of the failed norm, and L4 has no primitive for it; `HOMOICONICITY-SPEC.md` §§ at `:465-495` proposes one (`PARTY creditor MAY WAIVE (…)`, OPEN), and `SUBJECT-TO-NOTWITHSTANDING-SPEC.md:1173` classes waiver as a ledger-backed dispensation. A drafter who owns both texts can write it today with `RECORD`/`RECALL` (§3.2); what cannot be done is to read it from the norm log.                                                                                                                                                                                                                                               |
| **N10** | **Operand order inside a compound.**                                       | A `RAND`/`ROR` hands its left operand the whole event list and runs the right operand afterwards (`Machine.hs:1549-1559`), so a read in one operand sees the other's fates only if the other is on the left — whatever the timestamps. As-of reads do not help: the sibling's entry has not been written yet. Related to the pending tie-break ruling on same-event breaches in `RAND`/`ROR` (trigger word SEESAW, in another session). **Measured (§7): swapping the operands of the library contract flips the answer, with one event and with two distinct events.**                                                                                                                                                                                                                |
...
**N4 — a guard has no name for the contract-clock "now".**
`PROVIDED` could not say "within the last 365 days" because nothing in it names the current contract instant.
`NOW` exists and type-checks there, but it is the wall clock: `PROVIDED x EQUALS DATETIME_SERIAL NOW` compared an event at contract instant 5 against 740249.6… and silently did not match.
So the probe had to put the date in the action (`terminate 110`) and bind it.
...
A third hole, found in review on 2026-09-26: a named `b MEANS BREACH` shared by two `LEST`s breaches twice but writes **one** `norm breached` entry, because the breach is evaluated once and cached; the same contract with each `LEST BREACH` written inline writes two.
That entry is missing rather than unplaceable, so no "could not place" count would catch it.
...
The ledger printer renders every source other than `COMMIT` as `RECORD` (`jl4-core/src/L4/EvaluateLazy.hs:494-495`), so the probe's machine-observed entries print with the verb `RECORD` and `party=patron`, although the bracket does say `source=NORM`. The same default already prints every `NOTIFY` write as `RECORD`.
...
- The sketch claimed that as-of-contract-clock reads remove the ordering problem. They do not remove it between the operands of a compound, at the same instant or not (N10).
...
A second review, on 2026-09-26, put each ruling in front of an independent skeptic before the rulings bench was published, and changed six more things.

- §1 item 3 said a continuation's clock is the stamp of the event that reached it; that is true of `HENCE` only, and a `LEST` runs on the missed deadline.
- N10 said the ordering problem was about two fates at one instant. It is about operand order, and holds for distinct instants too.
- N4 said a guard has no name for "now". `NOW` exists and means the wall clock; the missing name is the contract clock's. N4's caching argument was dropped: context reads are already fingerprinted.
- §7 N5 cited the printer two lines early, and did not say that the source was visible or that `NOTIFY` is misprinted the same way.
- §7 N7 gained the shared-breach hole.
- N1 gained `DO` and the correct classification of `Waiting`; N9 now cites the `WAIVE` already proposed in `HOMOICONICITY-SPEC.md`.
