...
| RuleSpec (Axiom Foundation) | **FUTURE** — researched 2026-09-09; verdict: the closest structural match to L4's own design values found in this programme so far — cited, time-aware (`versions[]`/`effective_from` matches L4's own `EVAL … UNDER RULES EFFECTIVE AT` shape), a small boolean/arithmetic YAML expression language, compiled to Rust/WASM by a real, actively-maintained multi-jurisdiction corpus (2,600+ commits on `rulespec-us` alone). Not to be confused with PolicyEngine, which Axiom uses as a validation oracle and which is not itself a new target (`policyengine-core` is "Forked from OpenFisca-Core" **[E]**, so the existing shipped OpenFisca bridge is the relevant starting point, unverified for drift from current upstream) | `specs/research/AXIOM-RULESPEC-POLICYENGINE-RESEARCH.md`                                                 |
