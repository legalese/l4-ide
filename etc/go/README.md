...
2026-08-18 `corpus.modules` declares the whole set (in dependency order, existence required,
`corpus.main` and any `corpus.wizard` required to be members), it is exported as the
space-separated `GO_S_CORPUS_MODULES`, and it is what the g1 `GO_MODULES` and the gate digest
...
`p7-ladder`) and the per-module golden pairs of `p7-tnr`/`p7-mcp` still read `GO_S_CORPUS` alone;
