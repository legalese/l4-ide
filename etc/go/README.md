...
2026-08-18 `encoding.modules` declares the whole set (in dependency order, existence required,
`encoding.main` and any `encoding.wizard` required to be members), it is exported as the
space-separated `GO_S_ENCODING_MODULES`, and it is what the g1 `GO_MODULES` and the gate digest
