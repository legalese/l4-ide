...
const CATALOGUE = resolve(HERE, "../known-defects.json");
  ...
  if (r.reproduces) {
    ...
    process.stdout.write(
      `::error::known-defects[${group}]: ${d.id} NO LONGER REPRODUCES (observed ${r.observed}, expected ${r.expected}).\n` +
        ...
        `  ${d.description}\n` +
        `  DELETE this entry from etc/go/known-defects.json. A negative control that no longer\n` +
