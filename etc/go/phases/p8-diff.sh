...
# (subject.json comparison.surface_map). THIS IS THE ONE STAGE LICENSED TO READ
...
if [[ -z "$MAP" ]]; then
  go_skip "the '$GO_S_ID' sidecar declares no comparison.surface_map, so the §8 comparator has no pairing declaration to run over. Add it to $GO_S_DIR/subject.json; writing the map (which decisions correspond, over which shared fact slots) is agent work owned by the deposit runbook in .claude/skills/running-the-l4-pipeline/SKILL.md and by DENOVO-DIFF-ORACLE.md."
fi
if [[ ! -f "$MAP" ]]; then
  go_skip "the declared comparison.surface_map has not been deposited yet: $MAP is not a file. Producing it is agent work; deposit it and re-run this stage."
fi
