...
#
# The oracle is drift, not existence. The generator writes into the committed
# figures directory, so after it runs, a non-empty `git diff` over that
# directory means the committed figures were STALE — which is a FAIL, never a
# pass. The run never commits and never stages. (turbo's own figures test is a
# one-directional drift guard by its header's own admission, so it cannot
...
FIGURES="$GO_S_LADDER_FIGURES_DIR"
...
set +e
(cd "$GO_S_LADDER_NPM_DIR" && JL4_LSP_CMD="$JL4_LSP_CMD" npm run "$GO_S_LADDER_NPM_SCRIPT") >"$LOG" 2>&1
GEN_RC=$?
set -e
tail -5 "$LOG" || true
...
if [[ $GEN_RC -ne 0 ]]; then
  ...
  if grep -qE 'command not found|ENOENT|code 127|Cannot find module' "$LOG"; then
    go_skip "the ladder generator's toolchain is not installed in this checkout — $(grep -m1 -oE '(tsx: command not found|code 127|Cannot find module.*)' "$LOG" || echo "see $LOG"). Run 'npm ci' at the repo root first; the generator is 'tsx $(basename "$GO_S_LADDER_DEMO_ENTRY")' and tsx is a workspace devDependency." \
      --artifact "$LOG"
  fi
  go_receipt --status DEGRADED \
    --reason "npm run $GO_S_LADDER_NPM_SCRIPT exited $GEN_RC; see $LOG" \
    --artifact "$LOG"
  exit "$GO_EXIT_FINDING"
fi
