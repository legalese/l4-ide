...
// `dereference: true` is load-bearing, not tidiness. In this repo
// `.claude/skills/writing-l4-rules` is a git symlink to `skills/writing-l4-rules`
// (mode 120000), and cpSync's default is `dereference: false` -- so without this
// the "bundle" is a single symlink pointing back at the source checkout. Two
// things then go wrong at once, both silently: the published bundle carries no
// skill files at all and dangles on anyone else's machine, and the link-rewrite
// pass below walks *through* the symlink and writes its rewritten output into
// the TRACKED SOURCE, corrupting the skill it was asked to package. Measured on
// `unstable` 2026-09-11: one run turned four correct `../../../../doc/...` links
// into `../../../../.claude/doc/...` and left two files modified in git status.
