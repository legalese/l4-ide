...
for one contract and nothing else, asked the five questions, scored against `truth.json`. The raw
...
| readers      | two Claude models, recorded in `results.json` as `haiku` and `sonnet`. Each reader was a fresh, single-turn conversation with **no tools** and no other context: one artifact (plus `history.txt` for B and C), then the five questions. Each (contract, artifact, model) cell was run **twice** (`repeat` 0 and 1), so 4 × 3 × 2 × 2 = **48 readings, 240 scored answers**                                                                                                                                                                                                                                                                                                |
| judge        | an LLM judge scoring each answer **0 or 1** against the `truth.json` answer, with a one-line rationale per reading (the `rationale` field). Two scoring rules are visible in the rationales and matter for reading the numbers: an answer whose _verdict_ is "cannot tell" where the truth is definite scores **0** even if the reasoning before the retraction was right (`tenancy/A/sonnet/1`: _"states the right inference but then retracts to 'cannot tell' as its verdict — scored 0 per the rule"_); an answer that is **incomplete but asserts nothing wrong** scores **1** (`every-run-example/B/haiku/0`: _"omits the day-14 deadline (incomplete, not wrong)"_) |
| not recorded | the exact model identifiers behind `haiku` and `sonnet`, the wrapper prompt around artifact + questions, the judge's model and prompt, and the effort/thinking settings. `results.json` carries only what the scoring pipeline returned. A rerun should commit all four alongside the rows                                                                                                                                                                                                                                                                                                                                                                                 |
...
answer. This classification is mine, from the rationales, and a second reader of `results.json`
could shift a case or two between "incomplete" and "wrong"; the 25-vs-0-vs-1 column would not move.
...
(8 of 16) inferred that doing the owed act discharges it, which the list never says, and the
judge's "cannot tell = 0" rule took the rest. On `every-run-example`, the one contract where the
...
that was taken (`ELSE` vs `LEST`) or the arm we sit in. Sonnet reconstructed the branch from the
history on `contracts` (both repeats) and was credited for "at the start" on `tenancy`; haiku
scored once in eight readings — "cannot tell" five times, the section name alone twice. On `promissory-note` all four A readers scored
...
### 3.4 The BPMN's misses are the exporter's documented losses, read back as facts

Thirteen of C's fourteen misses are confident wrong answers, and each lands on a loss that
`C.fidelity.txt` for that contract already names:
...
4. **The pictures-as-text fail silently; the list fails loudly** (§3). Of the list's 30 misses, 25
   are the reader saying it cannot tell; of the BPMN's 14, 13 are a wrong answer given as a fact,
   and all of those sit on losses `C.fidelity.txt` already names. A reader of the exported BPMN
   who is not also shown the fidelity report will, on these contracts, be told that receipts wait
   for every tenant and that a refused payment was credited.
...
- **The judge is an LLM too**, and its "cannot tell = 0" rule is a choice. Scoring "cannot tell"
...
A readers the same calendar conversion as B and C; (iii) commit the model identifiers, prompts and
judge alongside `results.json`; (iv) a vision run, with B and C rendered, to separate _content_
from _drawing_; and then, still, (v) the reader experiment §7.3 actually asks for. None of these
is decided here.
