...
- `at —` marks a step with no clock. Three steps have none: a look before any event has arrived; the moment two parallel parts of a contract are combined — a `RAND`/`ROR`, or the members of an `UPON EACH` fork, which the contract runs as parallel parts; and a `BREACH` the rule declares outright (`LEST BREACH`), which is a verdict, not a look at an event.

## `--json`

`--json` prints the same answers as JSON, one object per trace, for a program to read. The keys are the section names: `standing`, `owed`, `discharging`, `breaching`, `advancing`, `passedOver`, `untried`, `nextDeadline`, `deadlineNotKnown` (the obligations the "not counting" clause names; usually empty), plus `contract`, `line`, `events`, `clock`, and, with `--steps`, `steps`. Times are numbers on the contract clock. The golden `jl4/examples/lts/expected/contracts.json`, taken with `--steps`, is a complete specimen.
