...
- **Every value a program would branch on is a fixed token, never a sentence.** The sentences are the plain listing's; here they appear only under `why` and `reason` fields kept for a human reading the file. The tokens, by key:
  ...
  - `owed[].kind`: `owed` (with `party`, `modal`, `action`, and `dueBy`/`dueWithin`/`remaining` as known) · `heldBack` (with `done`, `total`, `until`: `allHave`) · `notStarted` (with `source`) · `inBreach` · `lapsed` (both with `breach`).
  ...
  - `steps[].outcome.what`: `waiting` · `partyMismatch` · `actionMismatch` · `guardFailed` · `matched` · `expired` · `breached` · `joined` (with `operator`: `and`/`or`, and `result`: `fulfilled`/`breached`/`pending`) · `joinReleased` · `joinExpired` · `joinFailed` · `joinStalled`. Where a step moved on, `then` is `hence` · `lest` · `breach`.
