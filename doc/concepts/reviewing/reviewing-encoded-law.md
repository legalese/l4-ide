...
**MCP** ([Model Context Protocol](https://modelcontextprotocol.io)) is how AI assistants consume the corpus. Each deployed encoding exposes its exported decisions as tools that assistants like Claude call _instead of guessing_: the AI does the conversation, the reasoner does the law, and every answer is reproducible from the encoding and the facts. (The HTTP evaluation endpoint returns the full evaluation trace on request; the MCP tool returns the decision alone as of 2026-09-14 — `jl4-service/src/McpServer.hs` calls the evaluator with `TraceNone` — and citations are not yet passed through either transport.) This is our answer to hallucination — the assistant’s fluency, guard-railed by an engine whose every step is auditable. Dozens of encodings already run this way on our `jl4-service`, from tenancy agreements to WTO subsidy rules.
...
```bash
# TAG is the newest tag on https://github.com/legalese/prereleases/releases
TAG=unstable-20260810-2e183e5
gh release download "$TAG" --repo legalese/prereleases \
  --pattern 'l4-*darwin-arm64.tar.gz' --pattern 'SHA256SUMS'

# the release ships checksums; there is no reason not to use them
shasum -a 256 --ignore-missing -c SHA256SUMS
tar -xzf "l4-$TAG-darwin-arm64.tar.gz"

export L4="$PWD/l4-$TAG-darwin-arm64/l4"
```

Each archive carries `l4`, `jl4-lsp`, the standard `libraries/`, and a `BUILD-INFO.txt` naming the exact commit it was built from — **36–53 MB** compressed depending on platform. These are **prereleases cut from `unstable`**, not from the stable line, and they say so on the Releases page and again in `BUILD-INFO.txt`. The tag names the commit, so an old one stays downloadable after a newer is cut. **The shelf is cut by hand and has no cron behind it**, so it can sit well behind `unstable`; compare the release body's `commit` line against your checkout before relying on it, and note that a binary older than your checkout's standard library will fail to load a prelude it cannot parse — leave `JL4_LIBRARY_PATH` unset so it uses its own embedded copy. Fetched with `gh` or `curl` the binary is not quarantined; a macOS _browser_ download normally is, and the fix is `xattr -d com.apple.quarantine l4`.
...
| ------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **The standalone `l4`** — from [legalese/prereleases](https://github.com/legalese/prereleases/releases) | A download and a `tar -xzf`. No GHC, no Cabal, no compile. You still need the repository, Node.js and GraphViz. | **Running a `go` on your own machine.** Point `$L4` at the extracted binary and the driver takes it. **The shortest local route.** Cut from `unstable` by hand, so check the release body's `commit` line against your checkout.                                                                                                                            |
...
**Which of this was measured, and on what.** On `darwin-arm64`, on **5 August 2026**, against release `unstable-20260805-c873bb5` — a prerelease published on **l4-ide's own** releases page, since **deleted**, which is why the shelf at `legalese/prereleases` exists and why every link above points there instead: the archive's checksum matched the published `SHA256SUMS`; the extracted `l4` evaluated a file importing a bundled library from an unrelated working directory with `JL4_LIBRARY_PATH` unset; `go.sh` refused a bogus `$L4` by name; and `L4=<extracted>/l4 etc/go/go.sh run --subject regcf --encoding primary --through p3-check` reached `p0-preflight: PASS` with `l4 check` exiting 0 on both corpus files. The `linux-x64` and `win32-x64` archives were smoke-tested on their own build runners, and _not_ by anyone running a `go` on that platform.
