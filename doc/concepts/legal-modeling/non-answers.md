...
Real encodings do this at scale. The Regulation Crowdfunding rules that ship with L4 encode part 227 of title 17 of the United States Code of Federal Regulations, which commenced on 16 May 2016. They carry the dollar figures for each period since, and are asked for a figure "as at" a rule date. For a date before commencement there is no figure to give, because the rules did not yet exist — so the encoding refuses, with the sentence `no Regulation Crowdfunding figure exists before commencement on 2016-05-16`. Before refusals existed, that same gap was written as a blank — so the web form asked whoever was using the rules to supply a figure that does not exist.
