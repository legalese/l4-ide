...
_Settled on 7 September 2026, built on 8 September 2026: there is one quantifier word, `EVERY`, and a line of its own — `ONCE ALL HAVE` or `UPON EACH` — saying when the follow-on fires. The two are drawn, as shapes and as runs, under [Seen as a diagram](../../reference/regulative/EVERY.md#seen-as-a-diagram) on the reference page. The word after `EVERY` names the kind of party and narrows the group, as `Flatmate` does in the table. Which people are in the group is given as a list, after `IN` — `EVERY Flatmate f IN flatmates` — ruled and built on 8 September 2026. (Before that the same list went inside the `WHO` condition, as `WHO elem f flatmates`; that spelling is deprecated as of the same day, and still runs.)_
