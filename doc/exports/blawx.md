...
- a `STRING`-sorted field, argument or result. Blawx's attribute-type dropdown is a closed list —
  boolean, number, date, time, datetime, duration, list, and the categories you declared — so a
  string-typed field has no value type to be declared under. Write an enum
