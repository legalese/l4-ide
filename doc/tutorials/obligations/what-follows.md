...
One more thing worth noticing while a fixed figure and a blank are both in view: they are written the _same_ way. Step 1's ``Pay Alice `Ms Ng` 1500`` requires that figure; this page's blank `amount` requires nothing, because it names nothing yet. There is no special spelling for "this value, exactly" any more, whether it sits on the whole act or on one figure inside it — a defined rent, a rule's input, and a blank all read the same: a name that refers to something requires it, and a name that refers to nothing is filled in from the event.
