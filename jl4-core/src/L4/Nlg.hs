...
instance Linearize (Expr Resolved) where
  linearize expr = case carameliseNode expr of
    ...
    Record _ cell val isOfficial mHence -> hcat ([ text (if isOfficial then "commit" else "record"), lin cell, text "is", lin val ] <> maybe [] (\k -> [ text "hence", lin k ]) mHence)
    ReadCell _ mParty isOfficial cell -> hcat ([ text "recall" ] <> (if isOfficial then [ text "official's" ] else []) <> maybe [] (\p -> [ lin p, text "'s" ]) mParty <> [ lin cell ])
