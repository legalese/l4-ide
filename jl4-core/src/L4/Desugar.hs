...
carameliseExprWithContext ctx = carameliseNode >>> \ case
  ...
  Record     ann cell val isOfficial mHence -> Record ann (carameliseExprWithContext InertCtxNone cell) (carameliseExprWithContext InertCtxNone val) isOfficial (fmap (carameliseExprWithContext InertCtxNone) mHence)
  ReadCell   ann mParty isOfficial cell -> ReadCell ann (fmap (carameliseExprWithContext InertCtxNone) mParty) isOfficial (carameliseExprWithContext InertCtxNone cell)
...
rewriteFieldRefs fields self = go fields
  where
    go flds expr = case expr of
      ...
      Record ann c v off mh -> Record ann (go flds c) (go flds v) off (fmap (go flds) mh)
      ReadCell ann mp off c -> ReadCell ann (fmap (go flds) mp) off (go flds c)
