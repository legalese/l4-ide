...
spec = describe "prettyLayout re-emits @nlg annotations (#966)" $ do
  ...
  -- form the printer must emit would end at that @]@ and leave the rest of the
  -- line as stray source. Measured before the escape existed: the printed
  -- module did not re-parse.
  survivesPrinting "a line annotation containing a bare close bracket"
    ["\\]"]
    "DECIDE foo @nlg see note [3] here\n  IS 1\n"

  -- The same hazard one character further along, and the one this file
  -- originally missed: a line annotation may END in a backslash, because a
  -- backslash that cannot begin an escape is ordinary text and there is no
  -- closing herald for it to run into. Copied through unchanged it escapes the
  -- @]@ the printer appends, and the annotation never closes.
  --
  -- This shape is in the corpus — @ok\/nlg-percent.l4@ carries it deliberately
  -- — and it was the whole-corpus round-trip suite, not this file, that caught
  -- it: the printed module swallowed the next five lines into one annotation
  -- and stopped parsing. Worth stating plainly, because it is the argument for
  -- why the corpus-wide property earns its twelve minutes.
  ...
  -- A backslash that cannot begin an escape, mid-text, must survive exactly as
  -- written. Doubling it would decode to the same thing by luck but change the
  -- bytes, and the two spellings are not interchangeable to a reader diffing
  -- formatted source.
  survivesPrinting "a backslash that cannot begin an escape stays as written"
    ["[see s.3\\q here]"]
    "GIVEN r IS A NUMBER\n\
    \GIVETH A NUMBER\n\
