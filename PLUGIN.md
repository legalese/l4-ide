...
The `@desc` on each parameter flows into the generated OpenAPI schema and the MCP tool's `inputSchema`, so downstream LLMs know exactly how to construct a valid call. See the full working file at [skills/writing-l4-rules/assets/example-parking.l4](.claude/skills/writing-l4-rules/assets/example-parking.l4).
...
A wrapper script ships inside the skill at [skills/writing-l4-rules/scripts/validate.sh](.claude/skills/writing-l4-rules/scripts/validate.sh).
