...
2. **Section 1.3 has two distinct deadlines**, which I kept as two separate facts: the wellness
   ...
   quantity as the *confirmation* date and default the (never-queried) visit date to an early,
...
3. **"Still pending" (Section 1.1(3)).** I modelled the 1.1(3)/1.2 interaction as: the
   ...
   time (visit ≤ month 6 and confirmation ≤ month 7), *or* if the hospitalization itself occurs
   before month 7 (the confirmation deadline hasn't yet passed, so nothing has yet been breached).
   Only Q4 turns on this: the question says "I *had given* confirmation ... 8 months after the
...
4. **Section 2.1 vs. Q5 (self-inflicted injury).** The contract's Section 3.1 exclusions list
   ...
   accidental Injury," and a deliberate act against oneself is not an *accident*. I encoded a
...
5. **Section 2.2 vs. Section 4.1.1 (territorial scope) and Q4.** Section 4.1.1 says the policy
   ...
   territorial condition on where the resulting hospital *confinement* must occur for the benefit
...
6. **Causal exclusions vs. occupational status (Q9).** Section 3.1 excludes events "arising
   ...
   *hospitalization* arose out of, not the claimant's occupation in general. Q9 states the
...
8. **Section 4.2 (Arbitration)** is read as a set of procedural prerequisites to *litigating or
   recovering on* the policy once a dispute has already arisen (commence arbitration within 3
...
9. **365-day benefit cap and "confinement occurred" (Section 2.2).** These bound how *much*
   ...
   condition, whose failure can zero out the *entire* claim — they are defined as standalone rules
