% queries.pl
%
% Nine claims, c1..c9, one per benchmark question in fixtures/queries-blind.md.
% Each qN/0 succeeds iff covered(cN) succeeds under policy.pl.
%
% Standing preamble (applies to every claim below): the agreement is signed
% and the premium was paid on time, and every fact not called out by the
% question itself is set so that it satisfies coverage and triggers no
% exclusion, per the task instructions.

% Facts below are grouped by claim (c1..c9), not by predicate, so each
% claim_*/2 predicate's clauses are non-contiguous by design.
...
claim_hospitalization_month(c1, 4).
...
claim_hospitalization_month(c2, 4).
...
% ---------------------------------------------------------------------
% Q3: hospitalized for pneumonia 5 months after the policy's effective
% date, age 65. The wellness-visit/confirmation facts are left "none":
% at month 5 -- before either the month-6 or month-7 SS1.3 deadline --
% that condition is still pending, not yet failed.
% ---------------------------------------------------------------------
...
claim_wellness_visit_month(c3, none).
claim_wellness_visit_provider_qualified(c3, false).
claim_written_confirmation_month(c3, none).
...
% the wellness visit was given 8 months after the effective date (late --
% the SS1.3 deadline is month 7). "Traveling abroad" is read as meaning
% the confinement itself was outside the United States.
% ---------------------------------------------------------------------
...
% fraud or misrepresentation. A voluntary, self-inflicted act is read as
% neither a sickness nor an accidental injury (it is intentional, not
% accidental) -- ground = neither.
% ---------------------------------------------------------------------
claim_agreement_signed(c5, true).
claim_premium_paid_month(c5, 0).
claim_hospitalization_month(c5, 4).
...
% hospitalization occurred within the policy term; no fraud.
% ---------------------------------------------------------------------
...
% visit provided 6 months after the effective date; I was serving as a
% police officer at the time of hospitalization. Being a police officer
% at the time is a status, not a cause: the bite did not arise out of
% police service, so causes excludes police_service.
% ---------------------------------------------------------------------
...
claim_wellness_visit_month(c9, 4).
