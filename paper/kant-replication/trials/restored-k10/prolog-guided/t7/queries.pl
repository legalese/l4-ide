% queries.pl -- the nine benchmark questions (Kant et al. 2025, Appendix A.2),
% each translated into a claim against covered/1 from policy.pl.
%
% Standing preamble applied to every query: all conditions not mentioned in
% the question are set to values that satisfy coverage and trigger no
% exclusion. The agreement is signed and the premium is paid on time in
% every claim, per the paper's own prompts.

% Each claim_*/2 fact is defined once per query block below, so clauses for
% the same predicate are not contiguous in this file; declare them so as not
% to warn.
...
claim_hospitalization_ground(c2, accidental_injury).
...
% hospitalization; wellness-visit proof provided 6.5 months after the
% effective date?
% ---------------------------------------------------------------------------
q6 :- covered(c6).
...
% ---------------------------------------------------------------------------
% Q9: hospitalized due to my son biting me in the ankle; wellness-visit proof
% provided 6 months after the effective date; serving as a police officer at
...
claim_wellness_visit_month(c9, 2).
