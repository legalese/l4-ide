% queries.pl -- the nine benchmark queries as claim facts.
%
% Standing preamble (applied to every query, per fixtures/queries-blind.md):
...
%   follows)", and the agreement has been signed and the premium has been
%   paid on time.
%
% So every claim below sets claim_agreement_signed = true and a premium
% paid at month 0 (on time), and sets every fact not referenced by its
% question to a value that satisfies coverage and triggers no exclusion:
% no fraud/misrepresentation, a timely qualified wellness visit and
% confirmation, no dispute, a claim properly made, confinement in a US
% hospital, a policy term of 12 months (the contract's own one-year term,
% Sec. 4.6), and continuous confinement of a few days.

% claim_* predicates are defined out of predicate-name order below (grouped
% by claim instead), hence discontiguous.
...
% Tested: firefighting is an absolute exclusion (Sec. 3.1.3).
q1 :- covered(c1).
...
% Tested: 78 is below the age-80 exclusion threshold (Sec. 3.1.5).
q2 :- covered(c2).
...
% Tested: ordinary sickness, well inside every deadline and exclusion.
q3 :- covered(c3).
...
% the wellness visit given ("I had given confirmation", past perfect --
% read as: before the hospitalization) 8 months after the effective date.
% Tested: 8 > 7, so Sec. 1.3's confirmation deadline is missed, and by the
% time of this (later) hospitalization the condition has already failed
% rather than still being pending -- Sec. 1.2 cancelation.
q4 :- covered(c4).
...
% Q5: hospitalized for punching my own face to show off for friends; did
% not commit fraud or misrepresentation.
% Tested: a deliberate, self-inflicted act is neither a sickness nor an
% accidental injury (Sec. 2.1) -- the claim fails on the coverage grant
% itself, independent of the fraud/misrepresentation red herring.
q5 :- covered(c5).
...
% visit proof given 6.5 months after the effective date.
% Tested: skydiving is an absolute exclusion (Sec. 3.1.1), regardless of
% the age (79, under 80) and wellness timing (6.5 =< 7) both being fine.
q6 :- covered(c6).
...
% within the policy term; did not commit fraud.
% Tested: military service is an absolute exclusion (Sec. 3.1.2), regardless
% of the term and fraud reassurances.
q8 :- covered(c8).
...
% proof given 6 months after the effective date; I was serving as a police
% officer at the time of hospitalization.
% Tested: the injury did not arise out of police service -- it is
% incidental that the claimant happened to be a police officer at the
% time -- so the police-service exclusion (Sec. 3.1.4) does not reach a
% cause unrelated to that service.
q9 :- covered(c9).
