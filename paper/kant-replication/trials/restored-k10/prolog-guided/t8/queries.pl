...
% with all 21 claim_* facts defined. Facts drawn directly from the question
% text are set to match it; every other fact is set to a value that keeps
% the policy in effect and triggers no exclusion, per the standing preamble
% ("assuming all other conditions are met and no other exclusions apply")
% and the instruction to assume the agreement is signed and the premium
% paid on time.
%
% See NOTES.md for the judgement calls behind the non-obvious choices
% (hospitalization_ground for q5/q9, confined_in_us_hospital for q4, the
% hospitalization_month chosen for q4/q6, and the causes list for q9).
%
% Each claim_* predicate's facts are grouped by claim (one block per
% question) rather than by predicate, following the layout shown in
% inputs/TASK.md. Declared discontiguous accordingly.
...
claim_written_proof_of_claim_month(c1, 2).
claim_recovery_sought_month(c1, 5).
...
% ---------------------------------------------------------------------------
% Q2. Age 78 at the time of hospitalization.
% ---------------------------------------------------------------------------
q2 :- covered(c2).
...
claim_written_proof_of_claim_month(c2, 2).
claim_recovery_sought_month(c2, 5).
...
claim_written_proof_of_claim_month(c4, 9).
claim_recovery_sought_month(c4, 12).
...
% ---------------------------------------------------------------------------
% Q5. Hospitalized for punching own face to show off for friends; no fraud
...
claim_written_proof_of_claim_month(c5, 2).
claim_recovery_sought_month(c5, 5).
...
claim_written_proof_of_claim_month(c6, 8).
claim_recovery_sought_month(c6, 11).
...
claim_written_proof_of_claim_month(c8, 5).
claim_recovery_sought_month(c8, 8).
...
% ---------------------------------------------------------------------------
% Q9. Hospitalized due to being bitten in the ankle by claimant's son; proof
% of wellness visit provided 6 months after the effective date; claimant was
