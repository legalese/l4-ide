% queries.pl - the nine benchmark claims against policy.pl's covered(C).
%
% Standing preamble applied to every claim below: the agreement is signed and
% the premium has been paid on time; every fact not put in issue by the
% question is set to whichever value satisfies all conditions for coverage
% and triggers no exclusion, so that the only facts doing real work are the
% ones the question actually asks about. See NOTES.md for the reasoning
% behind each non-obvious value (in particular c4, c6, c7, c9).
...
claim_hospitalization_month(c1, 2).
...
claim_recovery_sought_month(c1, 3).
...
claim_hospitalization_month(c2, 2).
...
claim_recovery_sought_month(c2, 3).
...
% --- Q3: pneumonia, 5 months after effective date, age 65
q3 :- covered(c3).
...
claim_recovery_sought_month(c3, 3).
...
% --- Q4: fall while traveling abroad; confirmation of wellness visit given
%         8 months after the effective date (see NOTES.md for why the
%         hospitalization month is placed after month 8 here).
q4 :- covered(c4).
...
claim_recovery_sought_month(c4, 3).
...
% --- Q5: hospitalized for punching my own face to show off; no fraud or
%         misrepresentation
q5 :- covered(c5).
claim_agreement_signed(c5, true).
claim_premium_paid_month(c5, 0).
claim_hospitalization_month(c5, 2).
...
claim_recovery_sought_month(c5, 3).
...
claim_recovery_sought_month(c6, 3).
...
% --- Q7: heart attack; proof of wellness visit submitted 2 months after the
%         effective date; age 75
q7 :- covered(c7).
...
claim_recovery_sought_month(c7, 3).
...
claim_hospitalization_month(c8, 2).
...
claim_recovery_sought_month(c8, 3).
...
% --- Q9: son biting me in the ankle; proof of wellness visit provided 6
%         months after the effective date; serving as a police officer at
%         the time of hospitalization (see NOTES.md: this is occupation, not
%         causation, so police_service is not among the causes).
q9 :- covered(c9).
...
claim_recovery_sought_month(c9, 3).
