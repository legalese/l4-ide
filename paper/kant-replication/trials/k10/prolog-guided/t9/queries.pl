% queries.pl
%
% q1/0 .. q9/0 correspond to the nine questions in inputs/queries-blind.md,
% in order. Each succeeds exactly when covered/1 says "yes" for that
% question's claim.
%
% Standing preamble applied to every question (per inputs/queries-blind.md):
% assume all conditions not mentioned in the question are met and no
% exclusion not mentioned is triggered; the agreement is signed and the
% premium is paid on time. All 18 claim_* facts are defined for every claim.
...
claim_wellness_visit_month(c2, none).
...
% confirmation of wellness visit 8 months after the effective date.
% ---------------------------------------------------------------------
q4 :- covered(c4).
...
claim_wellness_visit_month(c5, none).
...
claim_hospitalization_month(c7, 6).
...
claim_wellness_visit_month(c8, none).
