% queries.pl
%
% Nine benchmark claims (Kant et al. 2025, Appendix A.2 / inputs/queries-blind.md).
% Each qN/0 succeeds exactly when covered(cN) holds under the facts given
% below it. Facts drawn directly from question N are set to the stated
% value; every other fact is set to a neutral value chosen so that, on
% its own, it does not prevent coverage and does not trigger an
% exclusion (per the standing preamble: agreement signed, premium paid
% on time, and "assuming all other conditions are met and no other
% exclusions apply"). See NOTES.md for the judgement calls behind the
% choices below (Q5's ground, Q9's cause, Q4's unstated hospitalization
% month, and the arbitration/recovery facts common to all nine).
...
claim_hospitalization_month(c4, 3).
...
% ---------------------------------------------------------------------
% Q9: hospitalized due to my son biting me in the ankle; proof of
% wellness visit provided 6 months after the effective date; serving as
% a police officer at the time of hospitalization.
% ---------------------------------------------------------------------
q9 :- covered(c9).
