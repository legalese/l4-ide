...
2. **Q5 — "punching my own face to show off for my friends."** The schema's `Ground` enum
   ...
   unintended *result* even though the act was intentional. I went with the stricter reading and
...
5. **Premium payment and fraud/misrepresentation are treated as presence-only, not
   ...
   field to compare against, so no numeric deadline check was added. Likewise `` `fraud month`
   `` and `` `misrepresentation month` `` cancel the policy via `isJust` alone, without comparing
...
7. **60-day recovery bar (Section 3.2.1) approximated in months.** The schema is month-granular
   ...
   arbitration clause above it), so every claim in `apply.l4` sets `` `dispute arisen` IS FALSE
   ``, both settlement/arbitration month fields to `NOTHING`, `` `valid arbitration award issued`
