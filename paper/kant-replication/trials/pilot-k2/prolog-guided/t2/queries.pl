% ---------------------------------------------------------------------------
% q1..q9 -- the nine benchmark questions, each defining its own claim facts.
%
% Standing preamble (applies to every question): assume all conditions not
% mentioned by the question are met and no exclusion not mentioned applies;
% the agreement has been signed and the premium has been paid on time.
%
% Facts each question does not mention are set to a neutral, coverage-
% favoring baseline: agreement signed, premium paid at month 1, hospitalized
% at month 1 (comfortably inside both the policy term and the Section 1.3
% grace period), age 40, no fraud/misrepresentation, a compliant wellness
% visit at month 1 with a qualified provider, no written confirmation yet
% supplied (i.e. Section 1.3 still pending), no dispute, and a 12-month
% (one-year) policy term matching Section 3.6.
% ---------------------------------------------------------------------------
...
claim_written_confirmation_month(c1, none).
...
claim_written_confirmation_month(c2, none).
...
% --- wellness visit given 8 months after the effective date?
q4 :- covered(c4).
claim_agreement_signed(c4, true).
claim_premium_paid_month(c4, 1).
claim_hospitalization_month(c4, 1).
...
claim_written_confirmation_month(c5, none).
...
% --- Q6: injured skydiving, age 79, wellness-visit proof provided 6.5
% --- months after the effective date?
q6 :- covered(c6).
claim_agreement_signed(c6, true).
claim_premium_paid_month(c6, 1).
...
% --- Q7: hospitalized for a heart attack, confirmation submitted 2 months
% --- after the effective date, age 75?
q7 :- covered(c7).
claim_agreement_signed(c7, true).
claim_premium_paid_month(c7, 1).
...
claim_premium_paid_month(c8, 1).
claim_hospitalization_month(c8, 1).
...
claim_written_confirmation_month(c8, none).
...
% --- proof provided 6 months after the effective date, serving as a police
% --- officer at the time of hospitalization?
q9 :- covered(c9).
claim_agreement_signed(c9, true).
claim_premium_paid_month(c9, 1).
