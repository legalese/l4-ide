...
\par\upshape\hfill---Lewis Carroll \cite{carroll1871looking}
\end{quote}
...
From the drafter's side, the legislative-engineering programme of Schwartz,
Bar-Siman-Tov and Gelbard
\cite{schwartz2023ilde,schwartz2025legislativeengineering} reaches the same
diagnosis --- drafters have only a word processor --- and proposes integrated
drafting environments with test scenarios authored beside the text, while
leaving rules-as-code as a pluggable layer; L4 is a candidate for that layer.
The lineage is worth stating: LEOS, the European Commission's Akoma
Ntoso-based drafting editor \cite{leos}, fixes document \emph{structure};
SimpLEX, the Israeli Ministry of Justice's environment that Schwartz et al.\
describe, places drafter-authored test scenarios beside that structure; L4
sits beside SimpLEX where SimpLEX sits beside LEOS, making those scenarios
executable so that they survive redrafting.
The Commission's own roadmap for LEOS shows where that seat is empty: a
2024 study catalogued 61 candidate ``smart functionalities'' for the editor
in seven categories and prioritised eleven, none of which executes a rule
\cite{fitsilis2024augmentedleos}; and the 2025 ``hybrid AI'' proofs of
concept that followed suggest references, definitions, recital--article
links and reporting templates over Akoma Ntoso, generate no text by design,
and execute nothing \cite{palmirani2025hybridleos}. Both leave the
verification category to structure and citation. An executable encoding
with drafter-authored tests is a smart functionality of exactly that
category, delivered the way those reports deliver everything else: as a
micro-service behind the editor.
...
expunged, interrogable as of any date. The capability is not mutation but its
dual---letting a constitutive \code{DECIDE} \emph{query the regulative
trace}---and that dual is now realised. L4 provides \code{RECORD} (append to a
party's own ledger), \code{COMMIT}/\code{ATTEST} (append to the shared
\emph{official} record), and \code{RECALL} (the projecting read, including
\code{<party>'s} cross-party and \code{OFFICIAL's} official-record forms). A
proof action's \code{HENCE} appends with \code{RECORD}, and a later \code{DECIDE}
reads the fact back with \code{RECALL}, so the record-then-read loop is the
deontic sequencing \mbox{\code{p HENCE RECORD HENCE q}} threaded through the
regulative chain; the writes append and never overwrite by construction, the
no-\code{UPDATE} discipline argued above made concrete. What remains is the
\emph{second} temporal axis: \code{resolve} graduating from this last-write-wins
fold into the bitemporal, supersession-aware projection \code{established(F) AS OF
(validTime, knownAt)}, where a correction is a superseding event rather than the
latest write, and a typed \code{EnvironmentState} schema linking each read's type
to the fact it recorded. Those two refinements---not the query capability
itself---are the work in progress flagged at the head of this appendix.
