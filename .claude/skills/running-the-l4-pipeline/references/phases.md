...
- [What runs over the committed encoding](#what-runs-over-the-committed-encoding)
...
## What runs over the committed encoding

`--encoding primary` is the **replay run**: the subject's committed encoding driven through every projection leg its descriptor declares. No encoding happens. SPEC.md's entry condition for its G1 capability — PRs #177 and #180 landed — is satisfied.
...
**Five of the seven stages that used to refuse now run.** `p1-ingest`, `p2-sweep`, `p3-encode`, `p4-forks` and `p5-gate` are the deposit path's declared members, and each validates a **deposit** rather than producing one: the sidecar's `natlang_sources`, `comparison` and `encodings` sections say where the deposit lives, and the stage reports `SKIPPED` when it is not there (a missing prerequisite), `DEGRADED` naming the rules that fired, or `PASS` over an artifact whose sha256 is on the row. The runbook is in SKILL.md; this table says what the stage's oracle does and does not reach.
...
| `p3-encode` | `l4 check` over every deposited module — the compiler's own verdict that the deposit is L4 the toolchain accepts — and, where the encoding declares a `roadmap`, that the roadmap validates and every unit is dispositioned. The roadmap is what makes **how much** of the source was encoded checkable at all: without it a module covering one chapter passes exactly as cleanly as one covering the Act | **the encoding.** Isomorphism is HG1's; the two mechanisable house rules run in `p3-check` over the run's resolved module set, which on the deposit path is this stage's deposit — so `p3-check` follows and applies them to what you deposited |
...
One stage exists as an entry point and cannot run, and since 2026-08-09 it is the only one — the driver's list is `UNIMPLEMENTED_STAGES=(p10-publish)`. `p10-publish` prints what it would do and what is blocking it, then exits 3; it is not a member of either declared stage list, so its absence cannot make a run incomplete. (`p8-verify` sat in this table for a driver-side reason until 2026-08-09; it is declared on both paths now — see its own section below.)
...
**Rungs 2 and 3 are unbuilt**: the R4 fork-space agreement/divergence sweep (a deposited fork register now exists to sweep — regcf's carries 26 forks — but the sweep itself is unwritten), and the external model checker, which waits on the LTS semantics. Since 2026-08-09 `p8-verify` is HG1-gated on both paths, like everything at or after p6 — and since R9 that is a DERIVED fact rather than two hand-kept lists.

**Declared, 2026-08-09.** The driver's base list is `PRIMARY_STAGES=(p0-preflight p3-check p6-tests p8-verify)` (subject legs append to it), `p8-verify` sits in `DEPOSIT_STAGES` too, and `--only p8-verify` runs it over the run's resolved module set: the committed encoding (measured PASS, 0 findings), or the deposit (measured DEGRADED — 2 `vacuous-guard` findings inside the de novo `the investor is an accredited investor`, which is the stage doing its job on run 2's encoding).

---

## The seven projection legs

R0 rules that "it renders but cannot execute" is a **defect**, not a caveat. A leg runs iff the subject's descriptor declares it. The statuses below are what that rule looks like when it is applied honestly rather than aspirationally; the status column is what the regcf primary-encoding run **measures**, quoted as the worked example, not what any subject's run is promised.

The heading's number is **kept as a locator, not as a count** — it is quoted by name elsewhere in the tree — and the table below has listed more rows than that for some time. `p7-catala` joined them on 2026-09-21; the AKN leg has its own section further down.

| leg    | what it emits                                                         | oracle                                                                                                                                                                                                                                                                                                                                                                       | status measured on the regcf primary run                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------ | --------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| BPMN   | one process per regulative rule (three, in the regcf corpus)          | byte-diff against the subject's declared goldens, then soundness and interchange                                                                                                                                                                                                                                                                                             | `DEGRADED` as measured — the wiring was unbuilt at measurement time. Since BUILT and MERGED (PR #198, 2026-08-03): vendor-neutral `businessRuleTask` + a decisionRef resolver checker; the leg re-measures on the next primary run                                                                                                                                                                                                                            |
| ladder | the committed SVG figures, regenerated                                | drift: a non-empty `git diff` over the figures directory is a **fail**, never a pass                                                                                                                                                                                                                                                                                         | `PASS` when a `jl4-lsp` is provisioned (explicit `JL4_LSP_CMD`, or discovered from a worktree's `dist-newstyle`); `SKIPPED` when none exists anywhere                                                                                                                                                                                                                                                                                                         |
...
| Catala | one literate `.catala_en` per module that carries an `@export`        | the external Catala toolchain, in three layers: `catala typecheck`, `catala proof`'s NoOverlappingExceptions half, and `clerk test` re-executing every emitted ` ```catala-test-cli ` block. The expected values in those blocks were computed by L4's evaluator (R7), so layer 3 is two independently written evaluators agreeing rather than Catala checked against itself | `PASS`/`execution` when all three layers pass over a non-zero test count. Not measured on regcf, which does not declare the leg — the subject that does is `chubb`, and it measures `DEGRADED`: `l4 export catala` refuses that module, naming five helpers with no `GIVETH`. **`l4 export catala`'s own exit code is not the gate** — it exits 0 on emissions `catala typecheck` rejects (smucclaw/l4-ide#958), which is the whole reason the leg shells out |

### Why the DMN leg needs a canonicalisation, and why that is not a fudge

The CLI export is **not** byte-identical to the committed golden. Measured on the regcf subject: `l4 export dmn regcf.l4` against its golden, both files are 3,248 lines and 23 of them differ, every one of the form `main.l4:<position>` in the golden against `regcf.l4:<position>` from the CLI, because `jl4/tests/DmnExport.hs:3212` typechecks goldens against an empty virtual file system so no source URI reaches the `@ref` renderer. (`diff | wc -l` is 92 — four output lines per changed line — which is the figure this sentence used to quote as a line count.)
