...
What each stage of the single-instruction pipeline is for, what it deposits as its interface to the next stage, which of them validate a deposit rather than producing one, and — for the one that still cannot run, and the one that runs but is not yet declared — exactly what is blocking it.
...
One stage exists as an entry point and cannot run: `p10-publish` prints what it would do and what is blocking it, then exits 3. `p8-verify` is listed with it because `go.sh plan` still does, not because it still refuses. Neither is a member of any milestone's declared stage list, so their absence cannot make a milestone incomplete.

| stage         | blocker                                                                                                                                                                                                                                                                                                                                                   |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `p8-verify`   | **it no longer refuses** — `l4 verify` landed and the script runs (§`p8-verify` below). It is listed here because `go.sh` still carries it in `UNIMPLEMENTED_STAGES` and no milestone declares it, so `go.sh plan` still prints it among the refusers. Declaring it is a driver change this leg does not own.                                             |
...
**What a `PASS` is worth.** Much less than "verification" suggests, and the receipt says so on the record. The analysis is propositional — every leaf is an opaque atom, so no numeric, interval, date or string contradiction is in range — and each `DECIDE` is read on its own without inlining callees, which in a corpus of many small named limbs is most of the corpus. Findings are sound; silence is not a consistency proof. `l4 verify --help` carries the full statement.

**Rungs 2 and 3 are unbuilt**: the R4 fork-space agreement/divergence sweep needs the fork register P4 does not yet produce, and the external model checker waits on the LTS semantics. P8 gates nothing in G0–G4.

**Caveat, present tense.** `etc/go/go.sh` still lists `p8-verify` in `UNIMPLEMENTED_STAGES` and `G1_STAGES` does not contain it, so `go.sh run --milestone g1` does not reach the script and `--only p8-verify` runs nothing. The script itself is complete and was measured by invoking it directly against a real run directory. Declaring it is a one-line driver change that this leg does not own.
