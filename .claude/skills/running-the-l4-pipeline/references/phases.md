...
| BPMN   | one process per regulative rule (three, in the regcf corpus)          | byte-diff against the subject's declared goldens, then soundness and interchange                                                                                              | `DEGRADED` — the mandated BPMN→DMN wiring is not built and has no checker                                        |
