...
| stage         | blocker                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `p1-ingest`   | **retrieval.** The bundle format landed 2026-08-02 (`specs/todo/single-instruction-demo/schemas/source-bundle.schema.json`, validator `etc/go/lib/register-validate.mjs`), so "nothing to write into" is no longer true. Fetching the source is an outward network act the orchestrator does not perform — it is **your** work, and the archive-fallback route belongs in it. No sidecar declares where a bundle lives yet. |
| `p2-sweep`    | **searching.** The register format landed 2026-08-02 (`schemas/external-modifications.schema.json`), and its `searches[]` section is where "what was searched, not only what was found" becomes checkable — a search must state its scope and, required, what it does **not** cover. Performing the sweep is a web-search act the orchestrator does not make; that is yours. Validate what you write.                       |
...
| `p4-forks`    | **nothing to fork.** R4 ruled 2026-08-02 and `schemas/fork-register.schema.json` landed the same day, enforcing R4's 1:1 map between a materialised fork and an `Interpretation` field. A fork is opened at a site in an encoding, and `p3-encode` produces none. Register **completeness** stays unfalsifiable — see below.                                                                                                |
| `p5-gate`     | Its condition is explicitly a judgement. Two of its five checks already run inside `p3-check`; the other three are not mechanisable. The two register joins gained a format and a validator on 2026-08-02, so they are exit codes now — but no P1/P2/P4 output exists to join over, and neither join was ever the judgement. A script cannot hold this gate — your checklist and HG1 do.                                    |
...
What they do not buy: any of the three stages running. P1 and P2 need the network; P4 needs an encoding.
