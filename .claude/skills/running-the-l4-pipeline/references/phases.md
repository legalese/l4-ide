...
| stage         | blocker                                                                                                                                                                                                                                                                                                                                             |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `p1-ingest`   | G2 entry is gated on **R4**, which is open. Independently, SPEC.md §8 records that P1 has no bundle schema or manifest format defined anywhere, so there is nothing for an ingest stage to write into and no acceptance condition to check it against.                                                                                              |
| `p2-sweep`    | Two blockers. It is a web-search stage, and this orchestrator makes no outward network request except the loopback deployment in the MCP leg. And SPEC.md §8 names the missing machine-readable external-modification register as one of the two highest-leverage gaps in the pipeline — without it, P5's "every entry disposed" cannot be checked. |
...
| `p8-verify`   | **R5** is open (which verifier goes first), there is no CLI footing at all, SPEC.md §5 gives P8 no component row and §6 gives it no pass condition. A stage here would be pure `UNVERIFIED` by construction. P8 gates nothing in G0–G4.                                                                                                             |
...
| MCP    | a deployable zip, and a loopback deployment when a service is running | HTTP 2xx plus a non-empty `.mcp` tool list                                                  | `SKIPPED` without a service                                                                                      |
...
`l4 export regcf.l4 --to dmn` is **not** byte-identical to the committed golden. Measured: 92 differing lines, every one of the form `main.l4:<position>` in the golden against `regcf.l4:<position>` from the CLI, because `jl4/tests/DmnExport.hs:3212` typechecks goldens against an empty virtual file system so no source URI reaches the `@ref` renderer.
