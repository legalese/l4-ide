...
| BPMN   | one process per regulative rule (three, in the regcf corpus)          | byte-diff against the subject's declared goldens, then soundness and interchange                                                                                                                         | `DEGRADED` as measured — the wiring was unbuilt at measurement time. Since BUILT on PR #198 (open): vendor-neutral `businessRuleTask` + a decisionRef resolver checker; the leg re-measures when #198 lands under this branch |
