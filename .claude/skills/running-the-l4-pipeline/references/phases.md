...
| `p3-encode` | `l4 check` over every deposited module. That is all it is: the compiler's own verdict that the deposit is L4 the toolchain accepts                                                                                                                                                                        | **the encoding.** Isomorphism is HG1's; the two mechanisable house rules run in `p3-check` over the run's resolved module set, which on the deposit path is this stage's deposit — so `p3-check` follows and applies them to what you deposited |
