...
export interface AiChatStartParams {
   ...
   *  credential (a WorkOS user-scoped API key, attenuated and short-lived),
   *  uploads the workspace bundle (attachments + active file + L4 import
   *  tree), POSTs the turn to the agent, maps its SSE back onto the same
   *  AiChat* events, and deletes the credential at end of turn. Requires
   *  Legalese Cloud sign-in (the credential is minted from the session). */
  cloudAgent?: boolean
}
