...
const allLive = new Map<NodeId, State>(
  leafIds(second).map((id) => [id, "live"]),
);
...
const allTrue = new Map<NodeId, UBoolValue>(
  leafIds(second).map((id) => [id, "TrueV"]),
);
...
const sceneCourtAbove = layout(
  fn,
  defaultViewSpec({ states: courtStates, connectiveStyle: "above-wire" }),
  tm,
);
const sceneCourtStraddle = layout(
  fn,
  defaultViewSpec({ states: courtStates, connectiveStyle: "straddle-wire" }),
  tm,
);
const sceneCourtBelow = layout(
  fn,
  defaultViewSpec({ states: courtStates, connectiveStyle: "below-wire" }),
  tm,
);
...
function shift(prims: ScenePrim[], dx: number, dy: number): ScenePrim[] {
  ...
  return prims.map((p) => {
    if (p.kind === "box")
      return { ...p, rect: { ...p.rect, x: p.rect.x + dx, y: p.rect.y + dy } };
...
function diff(a: Scene, b: Scene): Scene {
  ...
  prims.push({
    kind: "text",
    at: { x: W / 2, y: 30 },
    text: "Penal Code s 415, second limb (no property) — surplusage as a dead rung",
    anchor: "middle",
    state: "inert",
    tag: "title",
    size: 16,
  ...
  prims.push({
    kind: "text",
    at: { x: 44, y: y + 24 },
    text: "The court's reading (Fourth Interpretation)",
    anchor: "start",
    state: "inert",
    tag: "title",
    size: 15,
  ...
  prims.push({
    kind: "text",
    at: { x: 44, y: yB + 24 },
    text: "The applicants' reading (Second Interpretation)",
    anchor: "start",
    state: "inert",
    tag: "title",
    size: 15,
  ...
  prims.push({
    kind: "text",
    at: { x: W / 2, y: yCap + 18 },
    text: 'The "dishonest concealment" rung can never carry current ⇒ a don\'t-care ⇒ the minimiser deletes it ⇒ Explanation 1 is otiose.  (GD [30], [32])',
    anchor: "middle",
    state: "inert",
    tag: "note",
    size: 12.5,
...
const write = (name: string, scene: Scene) => {
  ...
  console.log(
    `wrote ${name}  (${scene.size.w.toFixed(0)}x${scene.size.h.toFixed(0)})`,
  );
};
