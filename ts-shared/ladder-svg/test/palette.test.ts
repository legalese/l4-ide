...
const prims: ScenePrim[] = [
  ...
  },
  {
    kind: "box",
    id: 4,
    // INERT, not live: a placeholder keeps `placeholderFill` only while it has no value of
    // its own — once pinned, the state wash wins (§19). A live placeholder therefore no
    // longer reaches `placeholderFill` at all, which is why this fixture had to change.
...
test("a FALSE box does not render as an UNKNOWN one — DESIGN §6 / §15.1 / §18", () => {
  ...
  const emit = (state: "dead" | "inert") =>
    sceneToSvg({ size: { w: 10, h: 10 }, prims: [box(state)] }, pal);
  ...
  assert.ok(emit("dead").includes(pal.deadFill), "and so must its wash");
...
test("every built-in palette separates dead from inert", () => {
  ...
  for (const [name, pal] of [
    ...
    assert.notEqual(
      pal.deadFill,
      pal.boxFill,
      `${name}: the dead wash must differ from the plain box fill`,
    );
  }
});
