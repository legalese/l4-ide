...
- **`jl4-core/libraries/hierarchy.l4`** (354 lines, new) — `DECLARE RoseTree OF a HAS value, children`, generic operations (`count nodes`, depth, map/flatten), `item` / `labeled` / `numbered` / `restartAt` constructors, and a `render outline` that assigns a marker per depth (upper-alpha, decimal, lower-roman, …). Handles the irregular sequences legal drafting actually produces — an inserted "2b", an explicit restart — without disturbing neighbouring numbering. Carries 14 `#ASSERT`s of its own; its golden shows the rendered outputs verbatim (`"b.i	fair wear and tear excepted"`, `"2b	Late payment"`, and the restart cases).
