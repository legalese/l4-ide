-- | @l4 export docassemble FILE@ — compile the decision-rule subset of an L4 file to
...
docassembleCmd opts = do
  ...
  case (opts.daOutput, opts.daPackage) of
    (Just _, Just _) -> do
      hPutStrLn stderr
        "l4 export docassemble: --package cannot be combined with --output: \
...
readSideInputs src = do
  ...
  letter <- if haveTmpl
    then do
      ...
      hPutStrLn stderr $
        "l4 export docassemble: assembling the letter from " <> tmpl
...
--
-- Regeneration over our own tree /prunes/ first ('prunePrevious'). It did not,
-- once: writing without deleting meant that regenerating after the @.l4@ was
-- renamed left the whole previous inner package in the tree, and
-- @[tool.setuptools.packages.find] where = [\".\"]@ then found both, so a wheel
-- declaring itself @docassemble.l4beta@ shipped an importable
-- @docassemble.l4alpha@ whose @l4_source_text()@ had no data file behind it —
-- squatting a namespace the distribution does not own, and removing it on
-- uninstall. Renaming the source is the designed trigger (the slug follows the
-- basename), and the command reported success throughout.
...
writePackageTree dir srcFile pkg = do
  ...
  ours <- guardClobber dir
  when ours (prunePrevious dir)
  ...
  hPutStrLn stderr ("l4 export docassemble: wrote " <> Text.unpack tree.ptDistName <> " to " <> dir)
  for_ (map (relPath . (.dafPath)) tree.ptFiles <> [srcRel]) \rel ->
    hPutStrLn stderr ("l4 export docassemble:   " <> (dir </> rel))
  pure tree
...
packageMarker :: Text
packageMarker = "# Generated by `l4 export docassemble --package`"

-- | Decide whether @dir@ may be written into, and report whether it already
-- holds a tree this command wrote (the caller then prunes it). Exits on
-- refusal; an absent or empty directory is fine and is not \"ours\".
guardClobber :: FilePath -> IO Bool
guardClobber dir = do
  isDir <- doesDirectoryExist dir
  if not isDir then pure False else do
    entries <- listDirectory dir
    if null entries then pure False else do
      ...
      unless ours do
        hPutStrLn stderr $
          "l4 export docassemble: refusing to write a package into " <> dir
            <> ": it is not empty and its pyproject.toml was not written by \
               \`l4 export docassemble --package`. Regenerating over a previous run is \
               \fine; overwriting anything else is not. Remove the directory, or \
               \name an empty one."
        exitFailure
      pure ours

-- | Remove the generated content of a previous run before regenerating, so a
-- regeneration is a replacement and not an accumulation.
--
-- Exactly two things are removed, and both are wholly generated: the
-- @docassemble\/@ subtree (every path under it comes from 'renderPackageTree',
-- including the embedded @data\/sources@ copy, all of which this run rewrites)
-- and root-level @*.fidelity.txt@ reports (the caller writes the current one
-- straight after). Everything else the user may have put in their own package
-- directory — a README, a LICENSE, a @tests\/@ directory — is left alone;
-- 'guardClobber' has already established that this tree is one we wrote, not
-- that we own every file in it.
prunePrevious :: FilePath -> IO ()
prunePrevious dir = do
  let inner = dir </> "docassemble"
  haveInner <- doesDirectoryExist inner
  when haveInner do
    stale <- listDirectory inner
    hPutStrLn stderr $
      "l4 export docassemble: replacing the previously generated package tree ("
        <> intercalate ", " stale <> ")"
    removePathForcibly inner
  entries <- listDirectory dir
  for_ [ e | e <- entries, ".fidelity.txt" `Text.isSuffixOf` Text.pack e ] \e ->
...
reportFidelity opts mDest report = do
  ...
  unless (null ns) $
    hPutStrLn stderr . Text.unpack $
      "l4 export docassemble: docassemble could not carry everything this module says — " <> tally
  ...
  case mDest of
    Just dest -> do
      ...
      hPutStrLn stderr ("l4 export docassemble: fidelity report written to " <> dest)
