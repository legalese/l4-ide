...
_employee_ is capped instead and takes a pay cut on leave days. Either way somebody bears the
...
- **F1 — is the harmonised $5,000 co-matching cap a lifetime total or fresh headroom?**
  ...
  concentrated on the largest families. The fork _opens_ on 1 October 2027; before that both
...
- **Baby Bonus rates before 1 January 2015.** The scheme has been re-rated repeatedly since 2001 and
  this encoding holds two cohorts' rates. Below that floor the _comparison_ functions stop with a
...
- **Preschool fee reductions and the BTO changes**, announced at the same Rally. The preschool
  measure is stated as a _target_ ("we aim to reduce fees to $150 a month" by 2030) rather than a
...
payment and **$5,000** is a co-matching _cap_ that depends on families saving:
...
Flat per-cohort arithmetic is the wrong tool for distributional questions — _which_ families gain,
...
_(The exporter also emits `definition_period = MONTH` for every variable, where an annual benefit
would more naturally be `YEAR`. Worth checking before an agency wires it into a live model.)_
...
**What has _not_ been done:** no domain expert has reviewed this against the sources; HG1 is waived
...
_definition_ too, so the printed module re-parses. **Across a module boundary only the call site is
...
Requested so that a reader can price the capability rather than guess at it. This section carries
only what was **measured**; what those measurements cost in money depends on a price list and a
billing arrangement, both of which change independently of this run, so they are stated separately
and with a date in **Addendum A**.
...
the deposit and this document. One model, `claude-opus-5`; no subagents, no workflows, so nothing is
attributable to fan-out.

| measurement                                |                              value |
| ------------------------------------------ | ---------------------------------: |
| Wall clock, first retrieval → final report | **90 min** (20:49:19Z → 22:19:25Z) |
| Pipeline stage execution, within that      |                               98 s |
| API requests                               |                                237 |
| Tool calls                                 |                                253 |
| Output tokens (incl. ~91,262 reasoning)    |                            283,153 |
| Cache-write tokens (1h TTL)                |                            832,674 |
| Cache-read tokens (hits)                   |                         76,274,229 |
| Fresh (uncached) input tokens              |                                474 |
| Web searches                               |                                  6 |
| Committed L4                               |     1,536 lines across six modules |

Three things about those numbers, because each one changes how they generalise:

- **Cache reads dominate by two orders of magnitude, and they are the price of a long single-context
  session** — not of the law. Re-reading a large worktree from cache is what keeps an hour-long
  encoding coherent, but it means consumption tracks **session length**, not statute size. A shorter
  subject in a shorter session is cheaper per unit of output than this run suggests, and a
  compaction resets the multiplier.
- **The cache-write line assumes the 1-hour TTL this session ran under.** A 5-minute TTL writes the
  same tokens at a lower multiplier.
- **Do not sum the transcript naively.** A raw sum over usage records double-counts, because one API
  request appears once per streamed record; `etc/go/lib/cost-ledger.mjs` dedupes by `requestId` and
  warns that the naive sum over-reports by ~2.2×. Every figure above is deduped, and the pipeline's
  own attested ledger (`go-run-cost-ledger.json`) agrees within the three minutes between its
  22:16:03Z snapshot and the end of the session.

**What is not in these numbers:** no domain-expert review (HG1 was waived, §9.2), no deployment, and
no human drafting time. They are the machine cost of one operator saying _"SG Child Support Package:
go"_ and reading what came back.
