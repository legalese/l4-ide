 ...
 * REQUIRED, but the service OMITS `enum` entirely on non-enum nodes (verified
 * against the live schema — a `number`/`boolean` node has no `enum` key at all).
 * Reading `p.enum.length` on such a node would throw. The renderer therefore
 * walks this corrected, all-optional view instead of the shared type.
 */
